...
**Next micro step: 6.5 — verify output failure/shutdown handling before full pipeline integration.** Microphone suspension is implemented; all 109 tests pass. Validate live self-activation and repeated requests using `python -m jarvis.capture.diagnostics --duration 90 --speak-confirmation`. The diagnostic uses a fixed confirmation; STT-to-LLM-to-TTS wiring remains in module 7. Recognition improvements remain deferred until the full pipeline works.
