...
**Next micro step: 6.4 — prevent self-activation by suspending capture/detection during response playback.** Local TTS and playback adapters are implemented and the default output stream completed a real smoke test; all 105 tests pass. Confirm audibility with the user. Output cleanup/failure cases already have adapter tests; stage-level shutdown and return to detection remain part of integration. Recognition improvements remain deferred until the full pipeline works.
